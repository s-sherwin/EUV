function [ScatteredOrders,R_hyp_pol] = evalAbsorber(blankStr,Nx,Nz,stack,materials,nk,coordsX,coordsY,...
...
%         coords = [coordsY, Z(1), X(1), Z(2), X(2), Z(3), X(3), Z(4), X(4), Z(1), X(1)];
    ...
    %%
    R_hyp_pol = zeros(length(inds),length(orders),2,2);
    for iScat = 1:length(inds) % Loop over scattering simulations
        %% Set material properties, lambda, and theta
        lambdaTheta = setLambdaTheta(lambdaTheta_full,inds(iScat),materials,nk,lambdaStr,thetaStr);
        ...
        for iLam = 1:size(lambdaTheta,1)
            for iOrder = 1:length(m)
                for iCGRP = 1:length(cgrp)
                    %%
                    outCols = [outCols; cgrp(iCGRP) m(iOrder) zeros(1,length(cols) - length(header) + 1)];
                end
            end
        end
        %%
        ...
        R_hyp_pol(iScat,:,1,1) = reshape(abs(R_TE(1:end/2)).^2,length(m),[])';
        R_hyp_pol(iScat,:,2,1) = reshape(abs(R_TE(end/2+(1:end/2))).^2,length(m),[])';
        R_hyp_pol(iScat,:,1,2) = reshape(abs(R_TM(1:end/2)).^2,length(m),[])';
        R_hyp_pol(iScat,:,2,2) = reshape(abs(R_TM(end/2+(1:end/2))).^2,length(m),[])';
    end
    
    %% Sum up over polarizations
    s = size(R_hyp_pol);
